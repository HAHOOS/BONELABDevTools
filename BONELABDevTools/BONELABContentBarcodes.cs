...
{
    internal static class BONELABContentBarcodes
    {
        /// <summary>
        ...
        /// This code is modified to satisfy the program's needs, which is to quickly get barcodes for specific things such as Spawnables. All the barcodes that were made as a property in or under the CommonBarcodes class were added to this dictionary and given a BarcodeType value.
        ...
        public static PalletObject[] GetObjects(BarcodeType type)
        {
            var barcodes = all.Where(x => x.Type == type);
            List<PalletObject> objects = [];
            foreach (var barcode in barcodes)
            {
                objects.Add(new PalletObject() { Barcode = barcode.Barcode, Type = barcode.Type, Title = barcode.Title, isBONELABContent = true });
            }
            return [.. objects];
        }

        public static PalletObject[] GetObjects()
        {
            List<PalletObject> objects = [];
            foreach (var barcode in all)
            {
                objects.Add(new PalletObject() { Barcode = barcode.Barcode, Type = barcode.Type, Title = barcode.Title, isBONELABContent = true });
            }
            return [.. objects];
    ...
    public class BONELABContent
    {
        ...
        public required BarcodeType Type;
